...
import { buildSubscription, type OrgSubscription } from "../lib/subscription";
...
export function useSubscription(): { data: OrgSubscription | null; loading: boolean } {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    let hospitalStatus: unknown = "trial";
    let usage: Record<string, unknown> | null = null;
    let ready = 0;
    const settle = () => { ready++; if (ready >= 2) setLoading(false); setData(buildSubscription(hospitalStatus, usage)); };

    const unsubHospital = onSnapshot(doc(db, "hospitals", hid), (snap) => {
      hospitalStatus = (snap.data() as Record<string, unknown> | undefined)?.subscription_status ?? "trial";
      settle();
    }, () => settle());
